...
## ✅ v1.6.15'te ÇÖZÜLEN (PUSH EDİLDİ — tag v1.6.15)

### 💀 B188 — VS çöküyor: Windows access violation (KÖK NEDEN BULUNDU)
`QThread.terminate()`. Worker'lar `subprocess.communicate()` içinde bloklu;
`quit()` no-op (event loop yok), `wait()` timeout, `terminate()` OS thread'ini
syscall ortasında öldürüyor → interpreter bozuluyor.
Tetikleyici: conda1 seçiliyken pipx silip JupyterLab launch etmek.
`faulthandler`: `<invalid frame>` @ `micromamba_installer.list_conda_packages`.

**Çözüm:** thread yerine child process öldürülüyor, thread terk ediliyor.
`kill_active_micromamba()` + `_retire_pkg_loader()` + `_abandon_worker()`.

> 🚨 **KALICI KURAL:** `terminate()` YASAK. Şu an `src/` içinde hiç yok.
> Kontrol: `Select-String -Path .\src\**\*.py -Pattern "\.terminate\(\)"`

### 💀 B189 — `Qt FATAL: QThread: Destroyed while thread is still running`
İptal edilen conda kurulumu pencereden uzun yaşıyordu, çünkü `WorkerThread.cancel()`
sadece `self._cancelled = True` yapıyordu — micromamba çalışmaya devam ediyordu.
Artık `cancel()` child process'i öldürüyor, `closeEvent` de kapanmadan önce
micromamba çocuklarını temizliyor.

### 🧹 B190 — pipx silmede gereksiz banner'lar
`Deleting → Creating → is ready! → deleted` yerine `Deleting → removed → Creating`.
`ensure_pipx_env(quiet_success=True)`.

### 💬 B191 — "conda install failed" yanıltıcı mesajı
Solver çakışması artık tanınıyor ve pin'lenen Python sürümü yazılıyor.
tensorflow + Python 3.13 örneği: conda-forge'da win-64 için sadece TF 1.x var
(Python ≤3.7 istiyor) → artık "pip ile kurmayı deneyin" diyor.

### 🔤 B192 — `â§– Starting` mojibake + boş kutu karakterler
- `Popen(text=True)` encoding belirtmiyordu → Windows ANSI kod sayfası UTF-8 çıktıyı
  bozuyordu. `encoding="utf-8", errors="replace"` eklendi
- Fontta olmayan spinner karakterleri `_clean_micromamba_line()` ile süzülüyor
- **UI dersi:** buton metinlerinde emoji kullanma (`⏭ ↑ ↓ 🗑` boş kutu çıkıyor),
  düz kelime kullan. Sabit genişlikler de dar geliyor ("Remove" → "temov")
...
2026-07-24'te aynı gün iki kez ısırdı:

- **tensorflow** → `ml` env'inde (3.14.5) "One or more packages could not be found on
  PyPI. Please check the package names" dedi. Paket adı doğruydu; 3.14 wheel'i yoktu.
  Kullanıcı yazım hatası aradı.
- **MLflow** → `ml` env'ine sorunsuz **kuruldu**, UI başlatılınca ayrı konsolda çöktü:
  `ImportError: cannot import name 'Traversable' from 'importlib.abc'`
  (3.14'te silinen isim; upstream mlflow/mlflow#24155, `has-closing-pr`).
  VS hiçbir şey göstermedi — Python 3.13.13 ile `nlp` env'i açılınca çalıştı.

**Tasarım kısıtı — neden basit bir liste tutmuyoruz:**
"Python 3.14'te MLflow bozuk" diye sabit kodlanmış bir uyarı bugün doğru, MLflow
düzeltilince yalan olur ve kimse geri dönüp silmez. Çözüm **kendi kendini güncelleyen**
olmalı: paket düzelince uyarı kendiliğinden kaybolmalı.

**Mimari kısıt:** Launcher uygulamaları `subprocess.Popen` ile başlatılıp bırakılıyor
(`stderr=DEVNULL` veya `CREATE_NEW_CONSOLE`). VS başlatılan uygulamanın çıktısını
**okumuyor**, dolayısıyla açıldıktan sonraki çökmeyi göremez. Bu yüzden hata yakalama
değil, **başlatmadan önce deneme** yaklaşımı seçildi.

### Faz 1 — Preflight import kontrolü (ÖNCELİK)

App tanımına isteğe bağlı alan:
```python
"preflight_import": "mlflow.server.fastapi_app",
```
Launch'a basılınca, `Popen`'dan önce:
```
<env>/python.exe -c "import mlflow.server.fastapi_app"
```
1-2 saniye sürer. Hata çıkarsa anlamlı mesaj, çıkmazsa normal başlatma.
Alan tanımlı değilse kontrol atlanır → mevcut uygulamalar etkilenmez.

**Mesaj (sürüm sabit kodlanmadan, hatanın kendisinden üretilir):**
```
MLflow bu ortamda başlatılamıyor
Ortam: ml (Python 3.14.5)
Hata: cannot import name 'Traversable' from 'importlib.abc'

Bu, paketin bu Python sürümüyle henüz uyumlu olmadığını gösteriyor.
Daha eski bir Python (3.12 / 3.13) ile bir ortam deneyebilirsiniz.

[Yine de başlat]  [İptal]
```
**"Yine de başlat" şart** — yanlış pozitifte kullanıcıyı engellememeli.

**Neden kendini günceller:** MLflow düzeltildiği gün import başarılı olur, mesaj
görünmez. Bakılacak liste, silinecek uyarı yok.

**Hangi uygulamalara eklenmeli:** ağır bağımlılığı olanlar — MLflow, Streamlit, Gradio,
JupyterLab, Datasette. Hafif olanlarda gecikmeye değmez.

### Faz 2 — Kurulum öncesi `requires-python` kontrolü

PyPI JSON API (`https://pypi.org/pypi/<paket>/json` → `info.requires_python`) okunur,
env'in Python sürümüyle karşılaştırılır, uyuşmazsa kurulumdan önce uyarı.

⚠️ **Sınırı bilerek kabul ediyoruz:** MLflow vakasını yakalamaz — metadata'sı 3.14'ü
destekliyor diyor, sorun beyanda değil kodda. Ama tensorflow gibi açıkça `<3.14` diyen
paketleri yakalar ve bedava sayılır.

### Faz 3 (opsiyonel) — Uzaktan bilinen-sorun listesi

GitHub'da bir JSON (`known_issues.json`) tutulup uzaktan çekilir → sürüm çıkarmadan
güncellenebilir. Faz 1+2 yetmezse düşünülür; bakım yükü olduğu için son çare.

**Dosyalar:** `src/gui/launcher_run.py` (preflight + dialog), `src/gui/launcher_ui.py`
(app tanımlarına `preflight_import`), kurulum kontrolü için `src/core/pip_manager.py`

**Öncelik:** 🔴 Yüksek — sessiz başarısızlık en kötü kullanıcı deneyimi; bugün
Bayram'ın kendisi iki kez yanlış yöne gitti

---

## 🆕 AÇIK BUGLAR (v1.6.15 test turunda bulundu)

### 🔴 B193 — Delete iki kez tetikleniyor
conda1 silindiğinde `delete_venv` ikinci kez çağrılıyor ve
`❌ Environment 'conda1' not found` veriyor. Log: 2026-07-23 10:58.
Zararsız görünüyor ama sinyal/çağrı yolu yanlış — muhtemelen hem worker'dan
hem context-menu sinyalinden çağrılıyor.

### 🟡 B194 — venv tarafında da yanıltıcı paket hatası (→ F206 ile birlikte çözülecek)
`ml` env'inde (Python 3.14.5) tensorflow kurmaya çalışınca
"One or more packages could not be found on PyPI. Please check the package names"
diyor. Gerçek sebep: TF'nin Python 3.14 wheel'i yok. B191'in conda için yaptığını
pip tarafında da yapmak gerekiyor.

### 🧪 TEST EDİLMEMİŞ (v1.6.15)
- **Skip Mirror butonu** hiç gerçek kurulumda basılmadı — `🌐 [2/5] trying mirror ...`
  logu doğrulanmalı
- **Cancel → `_abandon_worker`** yolu test edilmedi — uzun conda kurulumu iptal edilip
  `⛔ [Conda] killed N running micromamba process(es)` görülmeli, sonra VS kapatılınca
  Qt FATAL çıkmamalı
- Conda Mirrors listesi 120px yükseklikte — 5 satırın hepsi görünmüyor olabilir
...
### 🟡 F175 — Conda Channels Yönetimi (FAZ 1) — **MIRROR KISMI YAPILDI (v1.6.15)**

**Hedef:** Anaconda Navigator'daki gibi UI'dan channel ekle/kaldır/öncelik belirle.

**✅ v1.6.15'te yapılan (mirror yönetimi):**
- Settings → **Paths → Conda Mirrors** listesi (ayrı sekme değil, Paths içinde)
- Add / Move Up / Move Down / Remove / Defaults butonları
- Sıra kullanıcı tarafından düzenlenebiliyor, `settings.json` → `conda_mirrors`
- `DEFAULT_CONDA_MIRRORS`: prefix.dev, anaconda.org, TUNA, NJU, BFSU
- Otomatik rotasyon: bozuk metadata / ağ hatası → sıradaki mirror
- **Skip Mirror** butonu — yavaş mirror'ı beklemeden atla (conda env'lerinde görünür)
- Gerçek solver hatasında rotasyon yapılmıyor (boşuna bekleme yok)

**❌ Hâlâ açık (asıl channel yönetimi):**
- Channel ≠ mirror. Yapılan iş conda-forge'un **farklı sunucuları** arasında geçiş.
  Gerçek channel yönetimi (bioconda, pytorch, nvidia eklemek) hâlâ yok
...
- Pip için aynısı: extra-index-url, trusted-host
- Drag-drop sıralama (şu an Move Up/Down butonları var)

**Dosyalar:** `src/gui/settings_page.py` (mirror UI burada), `src/core/micromamba_installer.py`
(rotasyon + `get_conda_mirrors()`); channel yönetimi için `src/core/channel_manager.py` (yeni) gerekecek

**Öncelik:** 🟡 Orta — mirror kısmı acıyı aldı, channel yönetimi hâlâ değerli
