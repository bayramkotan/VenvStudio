...
class PackageExportMixin:
    ...
    def _freeze_cmd_hint(self, action: str):
        """Show the command that produced the package list for an export.

        Every export below starts from PipManager.freeze(); the rest is
        VenvStudio writing a file. There is no shell command that emits a
        Dockerfile, but there is one behind the list it is built from, and
        ...
        self._show_command_hint(action, _cmds.get(_et, "pip freeze"))
    ...
    def _export_dockerfile(self):
        ...
        if not freeze:
            return
        self._freeze_cmd_hint("Export Dockerfile")
...
"""
    ...
    def _export_docker_compose(self):
        ...
        if not freeze:
            return
        self._freeze_cmd_hint("Export docker-compose.yml")
...
"""
    ...
    def _export_pyproject(self):
        ...
        if not freeze:
            return
        self._freeze_cmd_hint("Export pyproject.toml")
...
"""
    ...
    def _export_conda_yml(self):
        ...
        if not freeze:
            return
        self._freeze_cmd_hint("Export environment.yml")
...
"""
    ...
    def _export_clipboard(self):
        ...
        if not freeze:
            QMessageBox.warning(self, "Warning", "No packages to export.")
            return
        self._freeze_cmd_hint("Copy package list")
