...
# B71: NoScrollComboBox and LANGUAGES used to be DEFINED here as well as
# in settings_common.py -- byte-identical copies, so nothing had broken
# yet. settings_common.py's own docstring says it exists precisely so
# several mixins can share these; keeping a second copy in the class that
# imports those mixins defeats it. The copies are gone; this is the one
# definition now.
from .settings_common import NoScrollComboBox, LANGUAGES
...
# N45: EditorsMixin was never imported, so settings_editors.py had never
# run at all -- the copy of those methods further down this file was the
# only one anyone had ever seen.
from .settings_editors import EditorsMixin
...
class SettingsPage(AppearanceMixin, PythonMixin, CatalogMixin, AdvancedMixin, ToolchainMixin, EditorsMixin, QWidget):
    ...
    # mixin's had never run. The live bodies have been moved into
    # settings_toolchain.py, where the file always said they belonged.
    # Nothing is defined here twice any more.
    ...
    # ── Editor Integration ────────────────────────────────────────────────
    #
    # N45 (Bayram, 2026-08-27): these six methods used to be defined HERE as
    # well as in EditorsMixin, and a class's own method wins over a mixin's --
    # so settings_editors.py has been dead code ever since it was split out.
    # Its own docstring says "moved from settings_page.py"; the move happened,
    # the deletion did not.
    #
    # The symptom was a whole feature quietly not existing: the Default-editor
    # column added to the mixin never appeared, because this copy (five
    # columns, no Default) was the one running. Two copies of a decision, for
    # the fifth time in one session -- and the only reliable cure is to keep
    # one.
    #
    # Now inherited from EditorsMixin:
    #   _setup_vscode_ui_section, _get_editor_venv_dir, _refresh_editor_list,
    #   _register_editor, _unregister_editor, _register_all_editors
